...
function mapOrderToBackendRequest(order) {
  return {
    ...
    financials: {
      subtotal: order.charges.subtotal,
      tax: order.charges.tax,
...
export function OrderProvider({ children }) {
  ...
  const createCustomerOrder = useCallback(async (payload) => {
    let createdOrder = null;

    // Store locally first so UI updates immediately
    setOrders((prev) => {
    ...
    try {
      if (createdOrder) {
        const backendPayload = mapOrderToBackendRequest(createdOrder);
        ...
        if (!resp.ok) {
          ...
          console.error('Failed to save order in backend:', resp.status, text);
        } else {
          const saved = await resp.json();
          console.log('Order saved in backend:', saved);
        }
      }
    } catch (err) {
      console.error('Error calling backend /api/orders:', err);
    }
